...
Only source goes in version control — `target/` (the compiled output) is
gitignored and regenerated by anyone who runs `cargo build`, never
committed. `Cargo.lock` is the one exception worth calling out: commit it
(this workspace builds a binary, and pinning exact dependency versions for
an application, unlike a library, is the recommended default) — it's
created the first time you run `cargo build` if it doesn't exist yet.

```sh
cd wireguard-studio
cargo build --release        # sanity check + generates Cargo.lock
git init
git add .
git commit -m "Initial commit: Rust port of the WireGuard config studio"
```

Then create the (empty) repo on GitHub itself -- either via the web UI, or
with the `gh` CLI:

```sh
gh repo create wireguard-studio --private --source=. --remote=origin
git push -u origin main
```

For every change after that: edit, `cargo build` (or `cargo check` for a
quicker syntax/type pass) to confirm it still compiles, commit, push. The
GitHub Actions workflow then re-verifies the build on all three OSes on
